...
class Environment:
    ...
    def redrawEnv(self, inputSignalRate, frequencyScreenPos, time):
        ...
        timeToSamplesConversion = 1.0 / (
                    self.screenSizeX / 2.0)  # This converts the time shown in the viewing window (1.0 second) into discrete data points for displaying
        ...
        for x in range(0, self.screenSizeX):
            ...
            amplitude = 70
            y = int(amplitude * math.sin(
                (inputSignalRate) * (2 * math.pi) * timeToSamplesConversion * x) + centerPlotLineOffset)
        ...
        if (len(outputPlotPoints) > 1):
            ...
            def fit_sin(tt, yy, timeToSamplesConversion):
                ...
                try:
                    ...
                    return {"amp": abs(A), "omega": w, "phase": p, "offset": c, "freq": f, "period": 1. / f,
                            "maxcov": numpy.max(pcov), "rawres": (guess, popt, pcov)}
                except:
                    return {"amp": 0, "omega": 0, "phase": 0, "offset": 0, "freq": 0, "period": 0, "maxcov": 0,
                            "rawres": 0}
